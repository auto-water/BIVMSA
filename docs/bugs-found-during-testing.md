...
### B3. hallo123 误报（V_decl 惰性 markdown 误判）　⚠ 待修复
...
- **修复建议**：① non_action 块不参与 U 命中 ② U3（dropper）/U8（指令级）加锚校验（图片/链接引用≠下载执行/指令注入）③ 落盘前校验 `non_action` 块不出现在 `unconditional_harmful`。
...
### E1. roster 696 块 V_decl backfill 风暴　⚠ 待修复
...
- **修复建议**：① `VDECL_CHUNK` 120→40 ② backfill 改为**分批补测**（缺失块按批，非逐块）③ Phase 0 块划分合并相邻同触发条件行。
